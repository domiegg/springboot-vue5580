...
<div>
	...
	<div class="list-preview" :style='{"width":"100%","padding":"0 10%","margin":"30px auto","position":"relative"}'>
		...
		<div class="category-3" :style='{"border-top":"2px solid","padding":"20px 0","flexWrap":"wrap","display":"flex","width":"100%","border-bottom":"2px solid","height":"auto"}'>
			<div class="item" :class="swiperIndex == '-1' ? 'active' : ''" @click="getList(1, '全部')" :plain="isPlain">
				<div :style='{"color":"inherit","fontSize":"18px"}'>全部</div>
			</div>
			<div class="item" :class="swiperIndex == index ? 'active' : ''" v-for="(item, index) in fenlei" :key="index" @click="getList(1, item[feileiColumn], 'btn' + index)" :ref="'btn' + index" plain>
				<img v-if="item.image" :style='{"width":"20px","margin":"0 5px 0 0","objectFit":"cover","display":"block","height":"20px"}' :src="baseUrl + (item.image?item.image.split(',')[0]:'')">
				<div :style='{"color":"inherit","fontSize":"18px"}'>{{item[feileiColumn]}}</div>
			</div>
		</div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"row-gap":"10px","padding":"24px 0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>知识名称：</div>
        <el-input v-model="formSearch.zhishimingcheng" placeholder="知识名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>适用对象：</div>
        <el-input v-model="formSearch.shiyongduixiang" placeholder="适用对象" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>地区：</div>
        <el-input v-model="formSearch.diqu" placeholder="地区" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>专家姓名：</div>
        <el-input v-model="formSearch.zhuanjiaxingming" placeholder="专家姓名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('zhishiku','新增')" :style='{"cursor":"pointer","border":"1px solid","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"none","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/zhishikuAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"50%","padding":"0 10px 0 0","color":"#999","flex":"1"}'>
				<div class="title">{{item.zhishimingcheng}}</div>
				<div class="title">{{item.zhishifenlei}}</div>
				<div class="title">{{item.shiyongduixiang}}</div>
		        ...
		        <div :style='{"padding":"0 10px","borderColor":"red","borderStyle":"solid","borderWidth":"0 0 0px 0","display":"inline-block"}' class="publisher">
		          ...
		          <span class="text">{{item.zhuanjiazhanghao}}</span>
		        </div>
		        <div :style='{"padding":"0 10px","borderColor":"red","borderStyle":"solid","borderWidth":"0 0 0px 0","display":"inline-block"}' class="like">
		      ...
		      <div class="desc" v-html="item.zhishineirong"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('zhishifenlei/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'zhishifenlei'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.shiyongduixiang != '') searchWhere.shiyongduixiang = '%' + this.formSearch.shiyongduixiang + '%';
        if (this.formSearch.diqu != '') searchWhere.diqu = '%' + this.formSearch.diqu + '%';
        if (this.formSearch.zhuanjiaxingming != '') searchWhere.zhuanjiaxingming = '%' + this.formSearch.zhuanjiaxingming + '%';
		if(!this.centerType){
			params['sfsh'] = '是';
		}
        if (this.curFenlei != '全部') searchWhere.zhishifenlei = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`zhishiku/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/zhishikuDetail', query: params});
      },
