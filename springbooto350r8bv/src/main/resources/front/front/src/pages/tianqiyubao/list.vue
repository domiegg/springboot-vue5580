...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"row-gap":"10px","padding":"24px 0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
	  ...
	  <el-button v-if="btnAuth('tianqiyubao','新增')" :style='{"cursor":"pointer","border":"1px solid","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"none","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/tianqiyubaoAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"50%","padding":"0 10px 0 0","color":"#999","flex":"1"}'>
				...
				<div class="title">城市:{{item.chengshi}}</div>
				<div class="title">天气:{{item.tianqi}}</div>
		      ...
		      <div class="desc" v-html="item.xiangxishuoming"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'天气',list:this.tianqiOptions,tableName: 'tianqi',check: -1})
	    this.fengxiangOptions = '北,东北,东,东南,南,西南,西,西北'.split(',');
		this.selectOptionsList.push({name:'风向',list:this.fengxiangOptions,tableName: 'fengxiang',check: -1})
	    this.fengsuOptions = '无风,软风,轻风,微风,和风,劲风,强风,疾风,大风,烈风,狂风,暴风,台风'.split(',');
		this.selectOptionsList.push({name:'风速',list:this.fengsuOptions,tableName: 'fengsu',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.tianqi != '') searchWhere.tianqi = this.formSearch.tianqi;
        if (this.formSearch.fengxiang != '') searchWhere.fengxiang = this.formSearch.fengxiang;
        if (this.formSearch.fengsu != '') searchWhere.fengsu = this.formSearch.fengsu;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`tianqiyubao/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/tianqiyubaoDetail', query: params});
      },
