...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"row-gap":"10px","padding":"24px 0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>专家账号：</div>
        <el-input v-model="formSearch.zhuanjiazhanghao" placeholder="专家账号" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
      ...
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>专业领域：</div>
        <el-input v-model="formSearch.zhuanyelingyu" placeholder="专业领域" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('zhuanjia','新增')" :style='{"cursor":"pointer","border":"1px solid","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"none","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/zhuanjiaAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"50%","padding":"0 10px 0 0","color":"#999","flex":"1"}'>
				<div class="title">专家姓名:{{item.zhuanjiaxingming}}</div>
				<div class="title">学历:{{item.xueli}}</div>
		      ...
		      <div class="desc" v-html="item.zhuanyelingyu"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'学历',list:this.xueliOptions,tableName: 'xueli',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.zhuanjiazhanghao != '') searchWhere.zhuanjiazhanghao = '%' + this.formSearch.zhuanjiazhanghao + '%';
        if (this.formSearch.zhuanjiaxingming != '') searchWhere.zhuanjiaxingming = '%' + this.formSearch.zhuanjiaxingming + '%';
        if (this.formSearch.xueli != '') searchWhere.xueli = this.formSearch.xueli;
        if (this.formSearch.zhuanyelingyu != '') searchWhere.zhuanyelingyu = '%' + this.formSearch.zhuanyelingyu + '%';
		if(!this.centerType){
			params['sfsh'] = '是';
		}
        ...
        this.$http.get(`zhuanjia/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/zhuanjiaDetail', query: params});
      },
