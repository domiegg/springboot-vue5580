			...
			<el-form class="center-form-pv" :style='{"padding":"10px 0px","margin":"0 0 10px","borderColor":"#ccc","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"#eee","borderWidth":"1px","display":"flex","fontSize":"inherit","borderStyle":"dotted"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0 10px","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","background":"none","display":"flex","width":"100%","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">专家账号</label>
						<el-input v-model="searchForm.zhuanjiazhanghao" placeholder="专家账号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="学历" prop="xueli">
						...
						<el-select clearable v-model="searchForm.xueli" placeholder="请选择学历" >
							<el-option v-for="(item,index) in xueliOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">专业领域</label>
						<el-input v-model="searchForm.zhuanyelingyu" placeholder="专业领域" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('zhuanjia','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0px 2px","fontSize":"14px","color":"#333","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('zhuanjia','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('zhuanjia','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"padding":"0px","borderColor":"#ccc","margin":"0 0 0px","borderRadius":"0px","background":"#eee","borderWidth":"1px 0px 1px 1px","width":"100%","borderStyle":"solid dotted dotted"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="content" label="状态">
						<template slot-scope="scope">
                            <el-switch
								...
								active-color="#19aa8d"
								inactive-color="#999"
								@change="(e)=>zhuanjiastatusChange(e,scope.row)">
                            </el-switch>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zhuanjia','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"#333","height":"auto"}'></span>
								浏览
							</el-button>
							<el-button class="edit" v-if=" isAuth('zhuanjia','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zhuanjia','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    init () {
        this.sfshOptions = "是,否,待审核".split(',');
          this.xueliOptions = "大专,本科,硕士研究生,博士研究生".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.zhuanjiazhanghao!='' && this.searchForm.zhuanjiazhanghao!=undefined){
            params['zhuanjiazhanghao'] = '%' + this.searchForm.zhuanjiazhanghao + '%'
          }
           ...
           if(this.searchForm.zhuanyelingyu!='' && this.searchForm.zhuanyelingyu!=undefined){
            params['zhuanyelingyu'] = '%' + this.searchForm.zhuanyelingyu + '%'
          }
    ...
    shBatchHandler(){
      this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
        ...
        this.$http({
          url: "zhuanjia/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
