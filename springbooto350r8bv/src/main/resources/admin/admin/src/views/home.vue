<template>
<div class="content" :style='{"minHeight":"calc(100vh - 200px)","padding":"30px","margin":"0 auto","alignItems":"flex-start","flexWrap":"wrap","background":"url() no-repeat center top / cover","display":"flex","width":"100%","fontSize":"16px","justifyContent":"space-between","height":"100%"}'>
	<!-- notice -->
	<!-- title -->
	<div class="text" :style='{"padding":"0 20px","margin":"0 auto 30px","borderColor":"#ceddee","color":"rgb(51, 51, 51)","borderRadius":"0px","textAlign":"center","background":"none","borderWidth":"0px","width":"100%","fontSize":"24px","borderStyle":"solid","fontWeight":"500"}'>欢迎使用 {{this.$project.projectName}}</div>
	<!-- statis -->
	<div :style='{"padding":"0","boxShadow":"0 0px 0px 0px rgba(100,100,100,.05)","margin":"0 30px 20px 0px","borderColor":"#fceaee","alignItems":"center","color":"#666","display":"flex","justifyContent":"flex-start","borderRadius":"3px","flexWrap":"wrap","background":"none","borderWidth":"0px","width":"100%","fontSize":"18px","borderStyle":"double","order":"1"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"0 10px 0 0","margin":"0 20px 10px 0","borderColor":"#19aa8d","alignItems":"center","textAlign":"center","display":"flex","justifyContent":"center","borderRadius":"25px","background":"none","borderWidth":"1px","width":"calc(20% - 20px)","borderStyle":"dashed","height":"90px"}' v-if="isAuth('nongzuowu','首页总数')">
			<div :style='{"width":"100px","alignItems":"center","background":"url(http://codegen.caihongy.cn/20230811/0eb67b67438049adae248456cd16752f.png) no-repeat left center","justifyContent":"center","display":"none","height":"100px"}'>
				<span class="icon iconfont icon-tongji7" :style='{"color":"#fff","fontSize":"32px","display":"none"}'></span>
			</div>
			<div :style='{"width":"auto","flexDirection":"row-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 5px","lineHeight":"24px","fontSize":"24px","color":"#f00","fontWeight":"500","height":"24px"}'>{{nongzuowuCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"#2f4050","height":"24px"}'>农作物总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"0 10px 0 0","margin":"0 20px 10px 0","borderColor":"#19aa8d","alignItems":"center","textAlign":"center","display":"flex","justifyContent":"center","borderRadius":"25px","background":"none","borderWidth":"1px","width":"calc(20% - 20px)","borderStyle":"dashed","height":"90px"}' v-if="isAuth('tianqiyubao','首页总数')">
			<div :style='{"width":"100px","alignItems":"center","background":"url(http://codegen.caihongy.cn/20230811/0eb67b67438049adae248456cd16752f.png) no-repeat left center","justifyContent":"center","display":"none","height":"100px"}'>
				<span class="icon iconfont icon-tongji5" :style='{"color":"#fff","fontSize":"32px","display":"none"}'></span>
			</div>
			<div :style='{"width":"auto","flexDirection":"row-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"#f00","fontWeight":"500","height":"24px"}'>{{tianqiyubaoCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"#2f4050","height":"24px"}'>天气预报总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 2 -->
	<div class="type2" :style='{"padding":"0px","margin":"0","display":"flex","justifyContent":"space-between","alignContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"100%","position":"relative","height":"auto","order":"10"}'>
		<div id="nongzuowuChart1" class="echarts1" v-if="isAuth('nongzuowu','首页统计')"></div>
		<div id="tianqiyubaoChart1" class="echarts2" v-if="isAuth('tianqiyubao','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.nongzuowuChat1();
		this.gettianqiyubaoCount();
		this.tianqiyubaoChat1();
	},
	methods:{
		// 词云
        ...
        this.$http({
            url: `nongzuowu/valueMul/zuowumingcheng?yColumnNameMul=zhongzhimianji,zhongzhishuliang,yujishoucheng`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
		...
		gettianqiyubaoCount() {
			this.$http({
				url: `tianqiyubao/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.tianqiyubaoCount = data.data
				}
			})
		},
		tianqiyubaoChat1() {
        ...
        var tianqiyubaoChart1 = echarts.init(document.getElementById("tianqiyubaoChart1"),'macarons');
        this.$http({
            url: "tianqiyubao/group/tianqi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                tianqiyubaoChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    tianqiyubaoChart1.resize();
                };
            }
        });
      })
...
<style lang="scss" scoped>
	...
	.calendar td.today .text {
				border: 1px solid rgba(25,170,141,.2);
				border-radius: 4px;
				flex-direction: column;
				color: rgba(25,170,141,1);
				background: rgba(25,170,141,.16);
				display: flex;
				width: 100%;
				justify-content: center;
				align-items: center;
